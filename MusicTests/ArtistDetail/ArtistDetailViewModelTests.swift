...
class ArtistDetailViewModelTests: XCTestCase {
    ...
    func test_onAppear_success() {
        ...
        let viewModel = ArtistDetailViewModel(
            repository: ArtistDetailMockRepository(
                artistDetailModel: ArtistDetailModel.mock(1)
            )
        )
        ...
        viewModel.$model.sink { result in
            ...
            case let .loaded(model):
                if model.tracks.count == 2 && model.tracks.map ({ $0.id }) == [ArtistDetailModel.mock(1).tracks[0].id, ArtistDetailModel.mock(1).tracks[1].id] {
